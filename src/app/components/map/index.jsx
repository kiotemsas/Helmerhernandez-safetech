...
const MapComponent = () => { 
  ...
  const filterRoutes = (rotr, cSearch) => {
    ...
    if (rotr.length >= 1)
      return rotr.filter((t) =>
        t.plateNumber.toLocaleLowerCase().includes(cSearch.toLocaleLowerCase()),
      );
  ...
  const handleMapClick = (e, data) => {
    ...
    let firstWord = data.plateNumber.substring(0, 1)
    ...
    const initialVehicles = async () => {
...

        {
          response.result.map( (key) => {
            ...
            {key.lastEventPosition != null ?            

              setData((dataMarkers) => [...dataMarkers, 
              
                {
                      ...
                      speed: key.lastEventPosition.position.speed, 
  ...
  useEffect(() => {  
    ...
    subscription.on('create', async (index) => {  
      ...
      if (index.attributes.vehicle) {  
 
        if (index.attributes.resultObject.position) {  

          setData(dataMarkers => dataMarkers.map((value) =>  value.id === index.attributes.vehicle.id ? { 
                    ...
                    <Box className="popcard">
                        ...
                        <Grid spacing={1} container>
                          ...
                          <Grid item lg={4} md={4} sm={4} >
                            ...
                            <Box className="">
                              <Typography variant="h6">{infoMarker.plateNumber}</Typography>
                              <Typography variant="p">{Number.parseFloat(infoMarker.speed).toFixed(2)} Km/h</Typography>
                            </Box>
